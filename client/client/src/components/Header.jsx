...
const Header = () => {
  ...
  return (
    <motion.div className='flex flex-col justify-center items-center text-center my-20' initial={{opacity:0.2,y:100}} transition={{duration:1}} whileInView={{opacity:1,y:0}} viewport={{once:true}}>
        ...
        <motion.div initial={{opacity:0,y:-20}} animate={{opacity:1,y:0}} transition={{delay:0.2,duration:0.8}} className='text-stone-500 inline-flex text-center gap-2 bg-white px-6 py-1 rounded-full border border-neutral-500'>
            <p>Best text to image genrator</p>
            <img src={assets.star_icon}/>
        </motion.div>
        <h1 className='text-4xl max-w-[300px] sm:text-7xl sm:max-w-[590px] mx:auto mt-10 text-center'>Turn text to <span className='text-blue-700'>image</span>, in seconds.</h1>
        ...
        <button className='sm:text-lg text-white bg-black w-auto mt-8 px-12 py-2.5 flex items-center gap-2 rounded-full' onClick={onClickHandler}>Genrate Images 
        ...
        <p className='mt-2 text-neutral-600'>Generated images fron imgify</p>
    </motion.div>
  )
}
